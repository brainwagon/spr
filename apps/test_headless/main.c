...
/* Fragment Shader */
spr_fs_output_t my_fs(void* user_data, const spr_vertex_out_t* interpolated) {
    (void)user_data;
    spr_fs_output_t c;
    /* Basic pass-through of interpolated color */
    c.color.x = interpolated->color.x;
    c.color.y = interpolated->color.y;
    c.color.z = interpolated->color.z;
    c.opacity.x = 1.0f;
    c.opacity.y = 1.0f;
    c.opacity.z = 1.0f;
    return c;
